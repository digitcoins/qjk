...
#ifdef WIN32
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) / "qjk";
#else
    fs::path pathRet;
...
boost::filesystem::path GetConfigFile()
{
    ...
    fs::path pathConfigFile(GetArg("-conf", "qjk.conf"));
...
boost::filesystem::path GetPeercoinConfigFile()
{
    ...
    fs::path pathConfigFile(GetArg("-bitcoinconf", "qjk.conf"));
...
void ReadPeercoinConfigFile(map<string, string>& mapSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No QJK config file is OK
...
boost::filesystem::path GetPidFile()
{
    ...
    fs::path pathPidFile(GetArg("-pid", "qjk.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong qjk will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    ThreadSafeMessageBox(strMessage+" ", string("qjk"), wxOK | wxICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "QJK:" << FormatVersion(PEERSHARES_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
...
{
    return MyGetSpecialFolderPath(CSIDL_STARTUP, true) / "qjk.lnk";
}
...
{
    return GetAutostartDir() / "qjk.desktop";
}
