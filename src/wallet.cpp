...
// QJK: optional setting to unlock wallet for block minting only;
...
// QJK: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // QJK: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // QJK: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    //  post-backup change.
#ifdef WIN32
                    if (!GetBoolArg("-avatar", true)) // QJK: not avatar mode; qjk: avatar mode enabled by default to avoid change being sent to hidden address
#else
...
// QJK: create coin stake transaction
...
// QJK: check 'spent' consistency between wallet and txindex
// QJK: fix wallet spent state according to txindex
...
// QJK: disable transaction (only for coinstake)
