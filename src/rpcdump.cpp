...
Value importprivkey(const Array& params, bool fHelp)
{
    ...
    if (fWalletUnlockMintOnly) // QJK: no importprivkey in mint-only mode
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    ...
    if (pwalletMain->IsLocked())
        throw JSONRPCError(-13, "Error: Please enter the wallet passphrase with walletpassphrase first.");
    if (fWalletUnlockMintOnly) // QJK: no dumpprivkey in mint-only mode
        throw JSONRPCError(-102, "Wallet is unlocked for minting only.");
    CSecret vchSecret;
    bool fCompressed;
...
Value exportpeercoinkeys(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "exportbitcoinkeys\n"
            "Add the Bitcoin keys associated with the qjk addresses to the Bitcoin wallet. Bitcoin must be running and accept RPC commands.");
