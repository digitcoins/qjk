...
// Copyright (c) 2016 The QJK developers
...
// Global state
//
static const uint256 hashGenesisBlockOfficial("0x0000026ac32a09849aa98d4c1b9d85d5baf2c16281a1b1857dc8f4e6b6bcaa59");
...
const string strMessageMagic = "QJK Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // QJK: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // QJK: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // QJK: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// QJK: find block wanted by given orphan block
...
// QJK: minter's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge)
{
    static int64 nRewardCoinYear = CENT;  // creation amount per coin-year 利息
	//if(nCoinAge/10000000<3)return 5689000000*COIN;
	//GetProofOfStakeReward(): create=--9223372036854.-775808 nCoinAge=50777006
	//int64 nSubsidy = (pow(1.000037957, nCoinAge)-1)*COIN;//日利0.000037957
    int64 nSubsidy = nCoinAge * 0.00013359196468 *COIN;//0.01395948*COIN / 365.2422;
	//int64 nSubsidy = 1000000*COIN;*COIN
...
// QJK: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // QJK: target change every block
    // QJK: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // QJK: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // QJK: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // QJK: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // QJK: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // QJK: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // QJK: track money supply and mint amount info
    ...
    // QJK: fees are not collected by miners as in bitcoin
    // QJK: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// QJK: total coin age spent in transaction, in the unit of coin-days.
...
// QJK: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // QJK: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // QJK: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // QJK: record proof-of-stake hash value
...
bool CBlock::CheckBlock() const
{
    ...
    // QJK: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // QJK: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // QJK: check switch from proof of work to proof of stake
    ...
    // QJK: check that the block satisfies synchronized checkpoint
    ...
    // QJK: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // QJK: verify hash target and signature of coinstake tx
    ...
    // QJK: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // QJK: getblocks may not obtain the ancestor block rejected
    ...
    // QJK: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "QJK", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "QJK", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Created in China Date:2016-03-26 - build is for public"; // "Created in China Date:2015-08-25 - build is for public";
        unsigned int nTimeGenesis=1458921600; 
        unsigned int nNonceGenesis= 802664;
        ...
        printf("QJK Genesis Block Found:\n");
        printf("genesis hash=%s\n", block.GetHash().ToString().c_str());
        printf("merkle root=%s\n", block.hashMerkleRoot.ToString().c_str());
        block.print();
     
        printf("End QJK Genesis Block\n");
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        if (!fTestNet)
            assert(block.hashMerkleRoot == uint256("0xe2c46306df5baf3ee1a4ffc2316eef6f9dcebe38b7215fce71889d38438dc544"));
        else
        ...
        // QJK: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // QJK: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // QJK: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // QJK: should not enter safe mode for longer invalid chain
    // QJK: if sync-checkpoint is too old do not enter safe mode
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 90) && !fTestNet)
    {
        nPriority = 100;
		strStatusBar = "";
        //strStatusBar = "WARNING: Checkpoint is too old. Please wait for the block chain to download, or notify developers of the issue (https://github.com/QJK/QJK/issues).";
    }

    // QJK: if detected invalid checkpoint enter safe mode
    if (Checkpoints::hashInvalidCheckpoint != 0)
    {
        nPriority = 3000;
        strStatusBar = strRPC = "WARNING: Invalid checkpoint found! Displayed transactions may not be correct! You may need to upgrade, or notify developers of the issue (https://github.com/QJK/QJK/issues).";
    }
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // QJK: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // QJK: record my external IP reported by peer
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload()); // QJK: immediate retry during initial download
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // QJK: send latest proof-of-work block to allow the
                        // download node to accept as orphan (proof-of-stake 
                        // block might be rejected by stake connection check)
                        // QJK: send latest block
                        vector<CInv> vInv;
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // QJK: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // QJK: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // QJK: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("QJKMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QJKMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("QJKMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("QJKMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // QJK: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running QJKMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadQJKMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadQJKMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadQJKMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d QJKMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadBitcoinMiner, pwallet))
                printf("Error: CreateThread(ThreadQJKMiner) failed\n");
            Sleep(10);
        }
    }
}
