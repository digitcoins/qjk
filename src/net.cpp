...
{
    // Wait for IRC to get it first - disabled with qjk
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "qjk " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are qjk dns seeds.
static const char *strDNSSeed[][3] = {
    // NOTE: Only testnet nodes are included in this build. If you are using this
    // for a real qjk offering, you will want to remove the "t" that
    // appears before the node name and IP
      {"seed1.qianjinka.com","seed2.qianjinka.com","seed3.qianjinka.com"}
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // QJK enables dns seeding with testnet too
    {
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  qjk is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    if(GetBoolArg("-genpos", false)){
        // QJK: mint proof-of-stake blocks in the background
